...
// EtcdResolver for grpc client loadbalance From "google.golang.org/grpc/resolver/manual"  重构
...
func (r *EtcdResolver) start() (chan<- struct{}, error) {
	...
	r.keyPrifix = BuildPrefix(ServerNodeInfo{Name: r.SrvName, Version: r.SrvVersion})
...
func (r *EtcdResolver) sync() error {
	...
	for i := range getResp.Kvs {
		addr := resolver.Address{Addr: SplitPath(string(getResp.Kvs[i].Key), r.keyPrifix)}
...
func (r *EtcdResolver) update(evs []*clientv3.Event) {
	for _, ev := range evs {
		...
		addr := resolver.Address{Addr: SplitPath(key, r.keyPrifix)}
		switch ev.Type {
		case mvccpb.PUT:
